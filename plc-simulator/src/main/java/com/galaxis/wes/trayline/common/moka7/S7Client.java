...
public class S7Client {
    ...
    public int Connect() {
        ...
        // In case the connection is not completely established (TCP connection + ISO
        // connection + PDU negotiation)
        // we close the socket and its IO streams to revert the object back to
        // pre-Connect() state
        if (!Connected) {
            if (TCPSocket != null) {
                try {
